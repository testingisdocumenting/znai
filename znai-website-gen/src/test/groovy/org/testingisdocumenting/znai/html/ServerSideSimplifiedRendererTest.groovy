...
class ServerSideSimplifiedRendererTest {
    ...
    void "should render simple page for crawl indexing"() {
        def searchEntries = new PageLocalSearchEntries(
                toc.tocItems[0], [
                new PageSearchEntry(new PageSectionIdTitle('PS0', [:]), [SearchScore.STANDARD.text('hello \' " <> [] & world')]),
                new PageSearchEntry(new PageSectionIdTitle('PS1', [:]), [SearchScore.STANDARD.text('of search')])])

        ServerSideSimplifiedRenderer.renderPageTextContent(searchEntries).should ==
                ServerSideSimplifiedRenderer.LOADING_INDICATOR +
                '<section id="page-content" style="max-width: 640px; margin-left: auto; margin-right: auto;">\n' +
                '<article>\n' +
