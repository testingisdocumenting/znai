 ...
 * @author mykola
 */
class WebuitFeaturesTesting implements StepReporter, StandaloneTestListener {
    ...
    void onStepStart(TestStep step) {
        report.add("> " + StringUtils.createIndentation(step.numberOfParents * 2) + step.getInProgressMessage().toString())
    }

    @Override
    void onStepSuccess(TestStep step) {
        report.add(". " + StringUtils.createIndentation(step.numberOfParents * 2) + step.getCompletionMessage().toString())
    }

    @Override
    void onStepFailure(TestStep step) {
        report.add("X " + StringUtils.createIndentation(step.numberOfParents * 2) + step.getCompletionMessage().toString())
    }
