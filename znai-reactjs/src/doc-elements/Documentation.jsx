...
export class Documentation extends Component {
  constructor(props) {
    ...
    const autoSelectedTocItem = {
      ...
      anchorId: page.tocItem.pageSectionIdTitles[0] ? page.tocItem.pageSectionIdTitles[0].id : null,
    };
  ...
  renderDefaultDocMode() {
    ...
    const searchPopup = isSearchActive ? (
      <SearchPopup
    ...
    const preview = docMeta.previewEnabled ? (
      <Preview
        active={true}
    ...
    const PreviewTrackerWrapper = docMeta.previewEnabled ? DiffTracking : React.Fragment;
  ...
  renderPresentationMode() {
    const { presentationRegistry, presentationSectionId, docMeta, pageGenError } = this.state;
  ...
  keyDownHandler(e) {
    ...
    if (e.code === "Slash" && !isSearchActive && mode === DocumentationModes.DEFAULT && !isFromInputElement) {
      ...
      mode === DocumentationModes.DEFAULT &&
      ((e.code === "Equal" && e.altKey) || (e.code === "NumpadAdd" && e.altKey))
    ) {
  ...
  onPageLoad(urlHistoryState) {
    ...
    const presentationRegistry = new PresentationRegistry(
      theme.elementsLibrary,
      theme.presentationElementHandlers,
      page
    );
  ...
  onUrlChange(url, urlHistoryState) {
    return this.getAllPagesPromise().then(
      (allPages) => {
  ...
  updateCurrentPageSection() {
    ...
    if (sectionTitlesWithNode.length !== 0 && autoSelectedTocItem.anchorId !== visible.idTitle.id) {
    ...
    function combineSectionTitlesWithNodes(pageSectionNodes) {
      ...
      return pageSectionNodes.filter(isNodeIdPresentInSections).map((n, idx) => {
    ...
    function isVisible(id) {
      const visibleWithForcedId = withVisibleTitle.filter((s) => s.idTitle.id === id);
