...
export class HtmlNodeDiff {
  beforeNodeRoot: HTMLElement;
  afterNodeRoot: HTMLElement;
  beforeNodeList: HtmlNodeFlattenedList;
  afterNodeList: HtmlNodeFlattenedList;
  diffResultRaw: any[];
  addedNodes: HTMLElement[];
  addedNodesContainers: HTMLElement[];

  constructor(beforeNodeRoot: HTMLElement, afterNodeRoot: HTMLElement) {
  ...
  scrollAddedIntoView(nodeWithScroll: HTMLElement) {
    if (this.addedNodes.length === 0) {
      return;
    }

    const isAnyChangeVisible = this.addedNodes.some((node) =>
      isNodeInViewport(nodeWithScroll, node)
    );
...
function isNodeInViewport(containerNode: HTMLElement, node: HTMLElement) {
...
function scrollIntoView(nodeWithScroll: HTMLElement, node: HTMLElement) {
  const containerRect = nodeWithScroll.getBoundingClientRect();
  const rect = node.getBoundingClientRect();

  const scrollDelta =
    containerRect.top - rect.top + 0.33 * containerRect.height;
  nodeWithScroll.scrollTop -= scrollDelta;
}

function compareListEntry(a: any, b: any) {
  return a.value === b.value;
}

const animationClassName = "znai-preview-change-tracker-indicator";

function removeAnimationClass(animation: any) {
  removeClass(animation.target, animationClassName);
}

function addAnimationClass(node: Node) {
...
function removeClass(node: Node, className: string) {
  const parts = getNodeClassName(node).split(/\s+/);
  setNodeClassName(
    node,
    parts.filter((p: string) => p !== className).join(" ")
  );
}
