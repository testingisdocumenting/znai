...
class DocumentationNavigation {
  ...
  fullPageUrl(relativePageUrl) {
    ...
    const relativePageUrlWithoutLeadingSlash =
      relativePageUrl.indexOf("/") === 0 ? relativePageUrl.substring(1) : relativePageUrl;
  ...
  buildUrl(id) {
    ...
    return this.fullPageUrl(id.dirName + "/" + id.fileName + (id.pageSectionId ? "#" + id.pageSectionId : ""));
  }
  ...
  extractPageLocation(url) {
    ...
    url = url.substring(1);
    if (url === getDocId() || !url.startsWith(getDocId())) {
      return index;
    }

    url = url.substring(getDocId().length);

    const parts = url.split("/").filter((p) => p !== ".." && p.length > 0);

    if (parts.length === 1) {
      return { dirName: "", fileName: parts[0], anchorId: anchorId };
    }
